...
for i in range(10, 32):
    ...
    # Reference is now 10dB
    caput(RFsp, 10, repeat_value=True, wait=True, timeout=10)
...
# Create a range for the plotting X-axis (10 to 31)
dB_range = range(10, 32)
...
for j in range(0, N):
    ...
    # Calculate the RMS values for all X and Y data arrays
    s = slice(10, 32)
    N_points = 22.0
    
    # Calculate Means
    mu_X1 = np.mean(X1[j][s])
    mu_X2 = np.mean(X2[j][s])
    mu_X3 = np.mean(X3[j][s])
    mu_X4 = np.mean(X4[j][s])
    mu_Y1 = np.mean(Y1[j][s])
    mu_Y2 = np.mean(Y2[j][s])
    mu_Y3 = np.mean(Y3[j][s])
    mu_Y4 = np.mean(Y4[j][s])

    # Calculate Standard Deviations (RMS)
    RX1 = sqrt(sum(np.power(np.subtract(X1[j][s], mu_X1), 2)) / N_points)
    RX2 = sqrt(sum(np.power(np.subtract(X2[j][s], mu_X2), 2)) / N_points)
    RX3 = sqrt(sum(np.power(np.subtract(X3[j][s], mu_X3), 2)) / N_points)
    RX4 = sqrt(sum(np.power(np.subtract(X4[j][s], mu_X4), 2)) / N_points)
    RY1 = sqrt(sum(np.power(np.subtract(Y1[j][s], mu_Y1), 2)) / N_points)
    RY2 = sqrt(sum(np.power(np.subtract(Y2[j][s], mu_Y2), 2)) / N_points)
    RY3 = sqrt(sum(np.power(np.subtract(Y3[j][s], mu_Y3), 2)) / N_points)
    RY4 = sqrt(sum(np.power(np.subtract(Y4[j][s], mu_Y4), 2)) / N_points)

    frms.write("%8.3f,%8.3f,%8.3f,%8.3f,%8.3f,%8.3f,%8.3f,%8.3f,%s\n" %
               (RX1, RX2, RX3, RX4, RY1, RY2, RY3, RY4, BPMname[j]))
    
    # Writing data to file (10 to 31)
    for i in range(10, 32):
...
# Plotting: Sliced to match dB_range
for j in range(0, N):
    f = plt.figure(figsize=(7, 9))
    plt.subplot(2, 1, 1)
    # Using [10:] slice and dB_range for correct alignment
    plt.plot(dB_range, X1[j][10:], "-o", label="Beam without Correction")
    plt.plot(dB_range, X2[j][10:], "-o", label="Correction Prediction")
    plt.plot(dB_range, X3[j][10:], "-o", label="Beam with Correction")
    plt.plot(dB_range, X4[j][10:], "-o", label="Beam with old SG Table")
    ...
    plt.subplot(2, 1, 2)
    plt.plot(dB_range, Y1[j][10:], "-o", label="Beam without Correction")
    plt.plot(dB_range, Y2[j][10:], "-o", label="Correction Prediction")
    plt.plot(dB_range, Y3[j][10:], "-o", label="Beam with Correction")
    plt.plot(dB_range, Y4[j][10:], "-o", label="Beam with old SG Table")
    plt.grid(True)
    plt.xlabel("Attenuation (dB)")
